...
def find_cutoffs(code, mode, static_mode=False):
    ...
    for i, line in enumerate(code.split("\n")):
        ...
        if (
            GLOBAL_STATEMENT_LINE.match(line)
            and not settings_statements
            and not other_statements
        ):
    ...
    cutoff1 = (
        max(leading_global_statements) + 1
        if leading_global_statements
        else int(def_statement)
    )
    cutoff2 = (
        (min(other_statements) if other_statements else max(settings_statements) + 1)
        if settings_statements
        else cutoff1
    )
...
def transform(functions, sketch_globals, sketch_locals, println, *, mode):
    ...
    try:
        ...
        if count_noncomment_lines(fake_settings_code) > 1:
            ...
            exec(
                compile(fake_settings_ast, filename=filename, mode="exec"),
                sketch_globals,
                sketch_locals,
            )
            ...
            if count_noncomment_lines(fake_setup_code) == 1:
                ...
                # compile the fake code
                exec(
                    compile(fake_setup_ast, filename=filename, mode="exec"),
                    sketch_globals,
                    sketch_locals,
                )
                # extract the results and cleanup
