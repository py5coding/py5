...
@overload
def begin_shape() -> ContextManager:
     ...
     * begin_shape(kind: int, /) -> ContextManager
...
@overload
def begin_shape(kind: int, /) -> ContextManager:
    """Using the `begin_shape()` and `end_shape()` functions allow creating more
    complex forms.
...
def begin_shape(*args) -> ContextManager:
    """Using the `begin_shape()` and `end_shape()` functions allow creating more
    complex forms.
...
@overload
def begin_record(recorder: Py5Graphics, /) -> ContextManager:
     ...
     * begin_record(recorder: Py5Graphics, /) -> ContextManager
...
def begin_record(*args) -> ContextManager | Py5Graphics:
    """Opens a new file and all subsequent drawing functions are echoed to this file as
    well as the display window.
...
@overload
def begin_raw(raw_graphics: Py5Graphics, /) -> ContextManager:
     ...
     * begin_raw(raw_graphics: Py5Graphics, /) -> ContextManager
...
def begin_raw(*args) -> ContextManager | Py5Graphics:
    """To create vectors from 3D data, use the `begin_raw()` and `end_raw()` commands.
...
def begin_contour() -> ContextManager:
    """Use the `begin_contour()` and `end_contour()` methods to create negative shapes
    within shapes such as the center of the letter 'O'.
...
def push_matrix() -> ContextManager:
...
def begin_camera() -> ContextManager:
...
def create_image_from_numpy(
    ...
    *,
    ...
    """
    return _py5sketch.create_image_from_numpy(
        array,
        bands=bands,
        dst=dst,
    )
...
def convert_image(obj: Any, *, dst: Py5Image | None = None, **kwargs: Any) -> Py5Image:
...
def convert_cached_image(
    obj: Any, force_conversion: bool = False, **kwargs: Any
) -> Py5Image:
...
def convert_shape(obj: Any, **kwargs: Any) -> Py5Shape:
